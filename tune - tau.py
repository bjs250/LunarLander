...
for seed in seeds:
    ...
    tauo = .001 # for soft update, taken from (Continuous control with deep reinforcement learning)
    tunes = [0.85,0.90,0.95,1.05,1.10,1.15]
    taus = [int(i * tauo) for i in tunes]
    for tune, tau in enumerate(taus):
        ...
        while episode < max_episodes and np.mean(score_bucket) < 200:
            ...
            if episode > 350 and cmr[-1] < 0:
                print("Rule 1")
                break

            if episode > 200 and cmr[episode-50] > cmr[-1]:
                print("Rule 2")
                break
            ...
            for step in range(max_steps):
                ...
                if len(replaybuffer) > memory_size and step % target_update_rate == 0:
                    ...
                    #mem_indices = np.random.choice(len(replaybuffer), memory_size, replace=False)
                    #memories = list()
                    #for i in mem_indices:
                    #    experiences.append(replaybuffer[i])
        ...
        suffix = str(seed) + "tau" + str(tunes[tune])
